...
const extractContextFromPath = (filePath) => {
  ...
  return {
    ...
    tema: temaMatch ? temaMatch[1] : null,
  };
};
...
const updateNormativaFile = async (autoEntries) => {
  ...
  const autoSectionRegex = new RegExp(
    `${AUTO_SECTION_START}[\\s\\S]*?${AUTO_SECTION_END}`,
  );
  const replacement = `${AUTO_SECTION_START}\n${buildAutoGeneratedSection(autoEntries)}\n${AUTO_SECTION_END}`;

  if (!autoSectionRegex.test(fileContent)) {
    throw new Error('No s’ha trobat la secció auto-generada delimitada per AUTO-GENERATED-NORMATIVA-START i AUTO-GENERATED-NORMATIVA-END al fitxer normativaContent.js.');
  }
...
const run = async () => {
  const normativaModule = await import(`${pathToFileURL(NORMATIVA_PATH).href}?update=${Date.now()}`);
  const { normativaContent = {}, autoGeneratedNormativa = {} } = normativaModule;
  const baseKeys = new Set(
    Object.keys(normativaContent).filter((key) => !(key in autoGeneratedNormativa)),
  );
  ...
  if (nextCount === 0) {
    console.log('✅ Totes les referències normatives ja estan generades (cap canvi necessari).');
    return;
  }
